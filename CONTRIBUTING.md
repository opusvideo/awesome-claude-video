...
Suggest a case or correction through an issue or pull request. English is the default language, with a complete Chinese version.
...
- A public prompt or implementation resource is optional. Leave `resources` empty when none is verified, and show only the original-post link beneath that demo. The README should not include missing-resource notices.
- Preserve creator attribution, model attribution, original links, and verification evidence in `cases.json`. Do not present creator-reported model use as independent reproduction.

## Covers and corrections

Use actual frames from the selected videos for the featured gallery, preserve their proportions, and link each cover to its original post. Keep the gallery to two rows and four columns.

For corrections, identify the original post and supply supporting evidence. Creators may request attribution corrections or removal through an issue. All works remain subject to their creators' rights; inclusion grants no new license.

---

# 贡献说明

欢迎通过 Issue 或 Pull Request 提交案例或更正。项目默认英文，并提供完整中文版。
...
- 不强制补齐提示词或实现方式。尚未核实资源时，将 `resources` 留空，演示视频下方仅保留原帖入口，README 不添加缺失说明。
- 在 `cases.json` 中保留作者署名、模型归因、原始链接和核验证据，不将作者声称的模型使用情况写成已独立复现。

## 封面与更正

顶部精选封面使用对应视频的真实截图，保持原有比例，点击后进入原帖。精选区保持两行四列。

提出更正时，请指明原帖并提供证据。创作者可通过 Issue 请求修正署名或移除作品。所有作品仍受原作者权利约束，收录不代表授予新的许可。
